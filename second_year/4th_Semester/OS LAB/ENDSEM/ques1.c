...
void woman_leaves(int id){
    ...
    if (sharedData.insideWoman == 0) {
        if (sharedData.mantry > 0) {
            printf("Restroom empty. now waiting men can enter.\n");
            sharedData.crntGender = 1;
...
int main() {
    pthread_t manThread[5], womanThread[5];
    int manID[5], womanID[5];
    ...
    sleep(50);
    for(int i=0 ; i<5 ; i++) {
        pthread_attr_destroy(manThread[i]);
    }
    for(int i=0 ; i<5 ; i++) {
        pthread_attr_destroy(womanThread[i]);
    }
